...
Table~\ref{tab:folds} uses a three-fold group allocation: every session is assigned to exactly one validation fold. A post-review audit of all 418 image records finds zero train/validation session intersections in each fold. Under the operator's identity confirmation, these are tyre-group-disjoint internal folds. An earlier appearance-similarity audit flagged the 70,000- and 90,000-mileage sessions as potentially identical, affecting folds 0 and 2; visual resemblance is not proof of identity, and the later operator confirmation states they are different tyres. We retain this provenance distinction rather than describe the folds as confirmed leakage. The audit verifies recorded allocations, not independent physical identity or every historical loader execution.
...
\label{eq:ordinal}
\end{equation}
We additionally report class-index mean absolute error, $\mathrm{MAE}=N^{-1}\sum_i|\hat y_i-y_i|$, with labels $0,1,2$, and QWK, $1-\sum_{ab}w_{ab}O_{ab}/\sum_{ab}w_{ab}E_{ab}$, where $w_{ab}=(a-b)^2/4$, $O$ is the observed confusion matrix, and $E$ is its chance-agreement counterpart derived from class marginals. These ordinal diagnostics penalise a two-category mistake more than an adjacent error. They are not errors in kilometres or tread depth. Post-review endpoint reconstruction uses all 60 epoch records for MobileNetV4, the former headline configuration, and ResNet-50, the frozen downstream classifier: two configurations, three folds, and three seeds. This is an illustrative audit, not a fresh architecture-selection exercise.
...
The geometry engine's deterministic resizing repair passed a Tesla T4 resume check with zero maximum parameter difference. This verifies the exercised interruption path, not every possible hardware failure. Immutable revisions and hashes identify the reviewed artifacts. The present paper regenerates its numeric tables and plots from those frozen local records; it does not rerun training or claim independent replication of the original experiments.
