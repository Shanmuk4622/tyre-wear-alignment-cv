...
The preparation audit starts with 888 raw files and removes 470 byte-identical duplicates, leaving 418 clean RGB photographs at $1152\times1536$ pixels. Ten pre-generated derivatives per original produce 4,180 additional files at $768\times768$ pixels. These derivatives are linked to their source images and are not independent observations. The images were collected manually in a single acquisition session. The limited size and acquisition scope of the dataset restrict the diversity of lighting, viewpoints, and environmental conditions represented.
...
\begin{table}[t]
\caption{Group-disjoint clean-image allocations. Validation images are reused for internal evaluation; these folds are not an external test set.}
\label{tab:folds}
\centering\small
...
\midrule
0 & 232 & 186 & 8 training / 4 validation groups\\
1 & 290 & 128 & 8 training / 4 validation groups\\
2 & 314 & 104 & 8 training / 4 validation groups\\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:folds} uses a three-fold group allocation: every session is assigned to exactly one validation fold. An allocation audit of all 418 image records finds zero train/validation session intersections in each fold. Under the operator's identity confirmation, these are tyre-group-disjoint internal folds. An earlier appearance-similarity audit flagged the 70,000- and 90,000-mileage sessions as potentially identical, affecting folds 0 and 2; visual resemblance is not proof of identity, and the later operator confirmation states they are different tyres. The recorded groups are therefore disjoint under the supplied identity labels; the earlier visual-similarity flag remains documented in the data provenance.

Classification training can use derivatives of training originals; validation uses clean held-out photographs. The dense-task branch uses clean originals only. Repeated seeds measure optimisation variation on fixed data. Validation-selected results reuse the evaluated fold for checkpoint selection; group disjointness does not make these selected results untouched test estimates.

\subsection{Manual Regions and Geometry Annotations}
The available manually annotated label map $M$ defines overlapping binary regions:
\begin{equation}
...
The point annotation interface fixes the three guide rows and records visibility state and horizontal left/right positions. The completion audit verifies 120 complete records and 720 visible coordinates, package/image hashes, and schema consistency. Two near-frame-edge coordinates were specifically inspected. These checks establish data integrity, not inter-annotator reliability: a second blinded annotator and repeated-label error estimate are unavailable. The all-``no visible issue'' observation fields are not used as healthy-reference or physical-wear labels. The labels must therefore be interpreted as one annotation protocol rather than an independent metrological standard.

\subsection{Endpoints and Analysis Units}
For classification, the validation-selected checkpoint is the earliest epoch attaining the maximum validation quadratic weighted kappa (QWK), using the original strict-improvement rule. The fixed-final checkpoint is epoch 60. We preserve both endpoints. They answer different questions, and selection on the evaluated validation data prevents the larger selected score from being treated as an untouched test result. Localisation and the matched geometry comparison use a fixed epoch-60 endpoint. Unless stated otherwise, numerical differences are descriptive; no population significance claim is inferred from repeated seeds, correlated points, or reused image sets. Missing metrics for a box-only model are undefined rather than zero.
...
\label{eq:ordinal}
\end{equation}
We additionally report class-index mean absolute error, $\mathrm{MAE}=N^{-1}\sum_i|\hat y_i-y_i|$, with labels $0,1,2$, and QWK, $1-\sum_{ab}w_{ab}O_{ab}/\sum_{ab}w_{ab}E_{ab}$, where $w_{ab}=(a-b)^2/4$, $O$ is the observed confusion matrix, and $E$ is its chance-agreement counterpart derived from class marginals. These ordinal diagnostics penalise a two-category mistake more than an adjacent error. They are not errors in kilometres or tread depth. Endpoint reconstruction uses all 60 epoch records for two configurations with distinct roles: MobileNetV4 illustrates the checkpoint sensitivity of a high-scoring recognition model, while ResNet-50 supplies the frozen classifier for the region interventions. Each configuration is evaluated over three folds and three seeds. This diagnostic selection supports interpretation of the existing experiments.
...
The explanation screen covers 18 initial candidates and ten shortlist confirmation runs, with 1,208 explanation rows and 35 method-faithfulness rows. A method survives the implemented gate if its randomisation sensitivity exceeds 0.05 and its insertion-minus-deletion area-under-curve value is non-missing. Surviving methods are ranked by that difference. There is no additional requirement that the difference be positive. The threshold is a recorded project heuristic, not a literature-derived validity boundary. We retrospectively repeat the initial-screen gate at 0.03, 0.05, and 0.10 on the same 34 method rows from 17 retained configurations, excluding the quarantined identity. No attribution maps are regenerated and no subsequent models are retrained. Thus the analysis tests initial eligibility sensitivity, not invariance of the complete explanation-guided pipeline.
...
YOLO polygon conversion is verified against the native manual regions. All 836 exported regions meet the required rasterised intersection-over-union (IoU) of at least 0.98; the minimum is approximately 0.980092. This tests the representation conversion, not annotation accuracy. Native masks remain the evaluation reference. For a semantic model, native-resolution sigmoid probabilities are thresholded at 0.5 independently for the tyre and tread channels. Each nonempty binary channel produces one enclosing axis-aligned box and one mask detection; its confidence is the mean probability over threshold-positive pixels. Empty channels produce no detection. These scored class-wise detections enter the same COCO evaluator used for the other backends; they are not a learned object-detection head. Box and mask average precision (AP) use IoU thresholds from 0.50 to 0.95. Region evaluation additionally reports IoU, Dice, and boundary F1 at a two-native-pixel tolerance.

To test downstream usefulness, each localisation run is paired with the frozen epoch-60 ResNet-50 classification checkpoint of the same fold and seed. Five inputs are evaluated: the full frame, predicted tyre crop, predicted tread crop, oracle tyre crop, and oracle tread crop. Crops include 5\% padding. A missing predicted region causes a recorded full-frame fallback. The classifier is not retrained on crops; therefore changes can reflect both localisation errors and input-distribution shift.
...
\subsection{Geometry Integration}
\label{sec:geometry}
The workstation keeps the analysed frame, portrait orientation, model settings, and predictions together. Deployed geometry uses seed 1, epoch 60, full-image resizing, ImageNet normalisation, and no test-selected rotation search. Local full-precision checks are adapter checks rather than a repetition of the mixed-precision benchmark.

A segmentation mask restricts the search for Canny edge supports. Deterministic consensus and least-squares fits estimate side lines $x=ay+b$ only when support is sufficiently long and consistent. The midpoint line is image-relative. Missing, crossed, clipped, or collapsed boundaries remain visible as rejected hypotheses. A side-view ellipse candidate additionally requires edge support and angular coverage; a detected tyre silhouette is not automatically interpreted as a rim. Temporal smoothing resets at invalid results and discontinuities. It can stabilise a display but cannot correct systematic localisation error.

The separate target-assisted software requires camera intrinsics, a ground-reference target, and a wheel-mounted target, using ChArUco detection \cite{charuco}. It has synthetic pose tests but no physical reference comparison for the current captures. It is an artifact extension rather than a fourth experimental contribution.
...
The geometry engine's deterministic resizing repair passed a Tesla T4 resume check with zero maximum parameter difference. This verifies deterministic recovery for the tested interruption path. Immutable revisions and hashes identify the source artifacts, and the manuscript tables and plots are regenerated from these frozen experimental records.
