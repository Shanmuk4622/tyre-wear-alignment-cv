...
import sys
sys.path.insert(0, str(__import__("pathlib").Path(__file__).resolve().parents[1]/"prototype"))
import bootstrap
...
def real_check():
    ...
    try:
        ...
        window.download_frame()
        window.export_fps.setValue(12.5)
        window.export_labels.setChecked(True)
        window.download_video()
        import json
        job=json.loads(window.export_job_path.read_text())
        assert job['fps']==12.5 and job['information_panel'] is True
        window.export_fps.setValue(6.)
        ...
        assert '12.5 fps' in window.export_status.text()
        assert cap.get(cv2.CAP_PROP_FPS) == 12.5
        ok, frame = cap.read()
        cap.release()
        ...
        print('PASS: real GPU inspection + independent CPU export, paired learned models, 4-frame 12.5 FPS portrait MP4 with information panel, PNG, controls responsive; '+str(destination))
    finally:
